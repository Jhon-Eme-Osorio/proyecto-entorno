...
$pdf->Cell(80,6,'LISTA DE CLIENTES',0,0,'C');
...
//SQL para consultas clientes
$cliente = $this->model->listaCliente();

foreach ( $cliente as $key):
        $pdf->SetX($nuevaPosX);
        $pdf->Cell(35,6,($key->Nombre),1,0,'C');
        $pdf->Cell(35,6,$key->Apellido,1,0,'C');
        $pdf->Cell(35,6,($key->Direccion),1,0,'C');
        $pdf->Cell(25,6,($key->Telefono),1,0,'C');
        $pdf->Cell(40,6,($key->Correo),1,1,'C');
    
 endforeach; 

//$pdf->AddPage(); //Agregar nueva Pagina

$pdf->Output('Reporte_Cliente_'.date('d_m_y').'.pdf', 'I'); 
